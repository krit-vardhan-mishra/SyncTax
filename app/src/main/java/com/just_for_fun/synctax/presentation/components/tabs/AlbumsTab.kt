...
fun AlbumsTab(
    ...
    onAlbumMenuClick: (String) -> Unit = {}
) {
    val albumsMap = songs.groupBy { it.album ?: "Unknown Album" }
    val albumList = remember(albumsMap) { albumsMap.entries.toList() }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        if (isGridView) {
            ...
            LazyVerticalGrid(
                ...
                items(albumList, key = { (album, _) -> album }) { (album, albumSongs) ->
                    val firstSong = albumSongs.first()
                    val albumArtUri = firstSong.albumArtUri.orEmpty()
                    ...
                    GridCard(
                        ...
                        title = album,
                        subtitle = artistName,
                        onClick = { onAlbumClick(album, artistName, albumSongs) }
                    )
                }
            }
        } else {
            ...
            OptimizedLazyColumn(
                ...
                items(albumList, key = { (album, _) -> album }) { (album, albumSongs) ->
                    ...
                    RowCard(
                        imageUri = albumArtUri,
                        // Swapping title/subtitle for better list view context (Album name as main title)
                        title = album,
                        subtitle = artistName,
                        detail = "${songCount} songs",
                        onClick = { onAlbumClick(album, artistName, albumSongs) },
                        onIconClick = { onAlbumMenuClick(album) }
                    )
                }
            }
        }
