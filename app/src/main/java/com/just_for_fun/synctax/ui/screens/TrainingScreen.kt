...
import com.just_for_fun.synctax.ui.viewmodels.HomeUiState
import com.just_for_fun.synctax.ui.viewmodels.HomeViewModel
import com.just_for_fun.synctax.ui.viewmodels.ModelTrainingStatus
import com.just_for_fun.synctax.ui.viewmodels.SongPlayCount
import com.just_for_fun.synctax.ui.viewmodels.TrainingSession
import com.just_for_fun.synctax.ui.viewmodels.TrainingStatistics
    ...
    Card(
        ...
        Column(
            ...
            if (modelStatus.lastTrainingTime > 0) {
                Text(
                    text = "Last trained: ${java.text.SimpleDateFormat("MMM dd, HH:mm").format(java.util.Date(modelStatus.lastTrainingTime))}",
                    style = MaterialTheme.typography.bodySmall,
...
private fun TrainingSessionRow(session: TrainingSession) {
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            Text(
                text = java.text.SimpleDateFormat("MMM dd, HH:mm").format(java.util.Date(session.timestamp)),
                style = MaterialTheme.typography.bodyMedium
            )
            Text(
