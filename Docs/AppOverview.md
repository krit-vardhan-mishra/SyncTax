...
This document provides a high-level overview of the SyncTax app architecture and components.
...
SyncTax is an offline-first Android music player and recommendation app with on-device machine learning. It's built as an experimental example of how to combine rich local media playback with lightweight personalized recommendations that run entirely on the device — no cloud required.
